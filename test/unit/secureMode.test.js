...
test('applySecureModeDefaults clears moderator settings when secure mode is enabled', () => {
  ...
  db.prepare("INSERT INTO settings(key, value) VALUES('moderatorCredentials', '[{\"username\":\"alice\",\"passwordHash\":\"hash\"}]')").run();

  const result = applySecureModeDefaults(db, { secureMode: true });

  assert.equal(result.secureMode, true);
  assert.equal(db.prepare("SELECT value FROM settings WHERE key = 'moderatorEnabled'").get().value, 'false');
  assert.equal(db.prepare("SELECT value FROM settings WHERE key = 'moderatorUsername'").get().value, '""');
  assert.equal(db.prepare("SELECT COUNT(*) AS n FROM settings WHERE key = 'moderatorPasswordHash'").get().n, 0);
  assert.deepEqual(JSON.parse(db.prepare("SELECT value FROM settings WHERE key = 'moderatorCredentials'").get().value), []);
});
